...
export const prizeStyles = {
  ...
  backgroundBlur1: 'absolute top-0 left-0 w-96 h-96 bg-turquoise/10 rounded-full blur-3xl',
  ...
  navButton:
    'absolute top-1/2 -translate-y-1/2 z-20 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-white/95 hover:bg-white border border-gray-200 shadow-lg flex items-center justify-center text-navy-blue hover:text-turquoise transition-colors disabled:opacity-40 disabled:pointer-events-none',
  ...
  dot: (active: boolean) =>
    `w-2.5 h-2.5 rounded-full transition-colors cursor-pointer ${
      active ? 'bg-turquoise scale-110' : 'bg-gray-300 hover:bg-gray-400'
    }`,
} as const;
