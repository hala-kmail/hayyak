...
export const cardStyles = {
  card: (hasOnClick: boolean, isLeader: boolean) =>
    `group relative bg-white rounded-3xl p-6 shadow-lg shadow-gray-200/40 border border-gray-100 transition-all duration-500 hover:-translate-y-2 hover:shadow-xl hover:shadow-gold/10 hover:border-gold/20 flex flex-col h-full text-right w-full ${
      hasOnClick ? 'cursor-pointer active:scale-[0.98]' : ''
    } ${isLeader ? 'ring-2 ring-gold/30' : ''}`,
  ...
  title: 'font-headline text-xl font-bold text-navy-blue mb-1 group-hover:text-gold transition-colors',
  ...
  progressValue: 'text-sm font-black text-gold',
  progressBar: 'h-2 w-full bg-gray-100 rounded-full overflow-hidden border border-gray-50',
  progressFill: 'h-full bg-gold rounded-full transition-all duration-1000 ease-out',
  ctaButton: 'inline-flex items-center gap-2 text-xs font-bold text-gold opacity-0 group-hover:opacity-100 transition-opacity',
  ctaIcon: 'w-3 h-3',
